...
On each run: prune `processed` entries with dates older than 30 days from today. Also prune files in `~/development/gmail-digest/output/messages/` with dates older than 30 days (based on the date prefix in the filename).
...
Read `~/development/gmail-digest/.processed.json`. Create empty if missing. Recreate empty if malformed. Prune `processed` entries older than 30 days. Prune `output/messages/` files older than 30 days.
...
Write a brief `~/development/gmail-digest/output/digests/YYYY-MM-DD-HHmm.md` noting:
...
8. Write to `~/development/gmail-digest/output/messages/{filename}` using an atomic write: write to `{filename}.tmp` first, then rename to `{filename}`.
...
Write to `~/development/gmail-digest/output/digests/YYYY-MM-DD-HHmm.md` using an atomic write (write to `.tmp` first, then rename). The filename uses the current local date and time (e.g., `2026-03-22-0700.md` for the 7am run). This preserves a history of digests rather than overwriting.
...
- Where the digest file is (`~/development/gmail-digest/output/digests/YYYY-MM-DD-HHmm.md`).

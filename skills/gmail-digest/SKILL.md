...
On each run: prune `processed` entries with dates older than 30 days from today. Also prune files in `output/messages/` and `output/digests/` (relative to current working directory) with dates older than 30 days (based on the date prefix in the filename).
...
Read `~/development/gmail-digest/.processed.json`. Create empty if missing. Recreate empty if malformed. Prune `processed` entries older than 30 days. Prune `output/messages/` and `output/digests/` files (relative to current working directory) older than 30 days.
...
Write a brief `output/digests/YYYY-MM-DD-HHmm.md` (relative to current working directory) noting:
...
### Step 7: Fetch and save messages
...
6. Add the message ID to the in-memory processed state with today's date as the value.

On fetch failure: report the error, skip the message, continue with remaining.

After all messages are fetched, write them to a single file `output/messages/YYYY-MM-DD-HHmm.md` (relative to current working directory, same timestamp as the digest) using an atomic write. Separate each message with `---`.

File format:

```markdown
# Messages — YYYY-MM-DD HH:MM
...
Write to `output/digests/YYYY-MM-DD-HHmm.md` (relative to current working directory) using an atomic write (write to `.tmp` first, then rename). The filename uses the current local date and time (e.g., `2026-03-22-0700.md` for the 7am run). This preserves a history of digests rather than overwriting.
...
- Where the digest file is (`output/digests/YYYY-MM-DD-HHmm.md` in the current working directory).
...
- **Atomic writes**: All writes to `.processed.json` and digest files must go through a `.tmp` file first, then rename. Message files in `output/messages/` also use atomic writes.
